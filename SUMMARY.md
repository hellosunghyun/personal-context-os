# 목차

이 레포는 가능한 한 순서대로 따라가도록 설계되어 있습니다.

처음 실행할 때는 **0. 시작**에서 방향을 잡고, **1. 첫 생성 흐름**을 따라가면 됩니다. 아래의 질문 묶음, 프롬프트, 템플릿, 예시는 진행 중 필요할 때 연결되는 참고 문서입니다.

---

## 0. 시작

- [시작하기](START_HERE.md)
- [왜 지금 필요한가](docs/00_why_now.md)
- [전체 실행 흐름](workflow/00_linear_flow.md)
- [첫 생성 완료](FINISH.md)

---

## 1. 첫 생성 흐름

1. [작업 공간 만들기](workflow/01_setup_workspace.md)
2. [선택 자료 모으기](workflow/02_collect_context.md)
3. [100문 100답 진행하기](workflow/03_answer_100_questions.md) - 실제 작성은 [100문 100답 기록지](templates/answer_log.md)에서 합니다.
4. [1차 인터뷰 진행하기](workflow/04_round1_interview.md)
5. [추가 질문 생성하기](workflow/05_generate_followups.md)
6. [2차 인터뷰 진행하기](workflow/06_round2_interview.md)
7. [파트별 최종 문서 초안 만들기](workflow/07_build_manual.md)
8. [초안을 합치고 검수하기](workflow/08_review_and_split.md)
9. [첫 생성 완료 상태 확인](FINISH.md)

---

## 2. 완성 후에 주로 보는 문서

- [업데이트 시스템 운영하기](workflow/09_update_system.md)
- [팀원과 공유하기](docs/08_team_sharing.md)
- [GPTs와 챗봇으로 확장하기](docs/09_gpts_and_bots.md)
- [품질 테스트 질문](tests/manual_quality_test.md) - 초안 검증에도 사용

---

## 3. 질문 원문과 예시

- [질문 사용법](questions/00_how_to_use_questions.md)
- [01. 현재 모습](questions/01_identity.md)
- [02. 살아온 과정](questions/02_life_story.md)
- [03. 가치관과 기준](questions/03_values.md)
- [04. 직관과 판단](questions/04_intuition_decision.md)
- [05. 일하는 방식](questions/05_work_style.md)
- [06. 소통과 피드백](questions/06_communication.md)
- [07. 에너지와 회복](questions/07_energy.md)
- [08. 관계와 경계](questions/08_relationships.md)
- [09. AI 답변 취향](questions/09_answer_taste.md)
- [10. 미래와 사용 설명서](questions/10_future_manual.md)

---

## 4. 진행 중 붙여넣는 프롬프트

- [00. 프로젝트 기본 맥락](prompts/00_project_context.md)
- [01. 자료 분류](prompts/01_material_intake.md)
- [02. 한 문제씩 진행](prompts/02_one_question_mode.md)
- [03. 1차 인터뷰](prompts/03_round1_interview.md)
- [04. 추가 질문 생성](prompts/04_generate_followups.md)
- [05. 2차 인터뷰](prompts/05_round2_interview.md)
- [05A. 미니 Active Context 만들기](prompts/05a_build_minimal_active_context.md)
- [06. 파트별 최종 문서 초안 생성](prompts/06_final_manual_deep.md)
- [06A. 정체성/가치/판단 초안](prompts/06a_build_identity_values.md)
- [06B. 일/소통/에너지/관계 초안](prompts/06b_build_work_communication.md)
- [06C. LLM 답변 취향/리스크/성장 초안](prompts/06c_build_llm_risks_growth.md)
- [06D. Active Context 초안](prompts/06d_build_active_context.md)
- [06E. 팀원용 1페이지 초안](prompts/06e_build_team_share.md)
- [06F. 공개용 프로필 초안](prompts/06f_build_public_profile.md)
- [06G. 파트 초안 합치기](prompts/06_merge_manual_outputs.md)
- [07. 저자성 검수](prompts/07_authorship_review.md)
- [08. 버전 분리](prompts/08_split_versions.md)
- [09. 업데이트](prompts/09_update.md)

---

## 5. 기록지와 템플릿

- [자료 목록](templates/source_register.md)
- [100문 100답 기록지](templates/answer_log.md)
- [인터뷰 요약](templates/interview_summary.md)
- [최종 사용 설명서 골격](templates/final_manual.md)
- [Active Context](templates/active_context.md)
- [답변 취향 로그](templates/answer_taste_log.md)
- [사건 기반 업데이트 로그](templates/event_update_log.md)

---

## 6. 개념과 주의사항

- [이 가이드는 무엇인가](docs/00_what_is_this.md)
- [왜 맥락이 중요한가](docs/01_why_context_matters.md)
- [도구와 진행 방식](docs/02_tools_and_modes.md)
- [보안과 경계](docs/03_security.md)
- [MBTI로 대체할 수 없는 이유](docs/04_why_not_mbti.md)
- [다른 방법론과의 차이](docs/05_method_comparison.md)
- [사용 예시](docs/06_use_cases.md)
- [단점과 트레이드오프](docs/07_limitations.md)
- [팀원과 공유하기](docs/08_team_sharing.md) - 완성 후 선택
- [GPTs와 챗봇으로 확장하기](docs/09_gpts_and_bots.md) - 완성 후 선택

---

## 7. 예시와 테스트

- [최소 예시](examples/00_minimal_run.md)
- [텍스트 진행 예시](examples/01_text_mode_example.md)
- [음성 진행 예시](examples/02_voice_mode_example.md)
- [최종 문서 일부 예시](examples/03_final_manual_excerpt.md)
- [팀 대화 예시](examples/04_team_conversation_example.md)
- [Active Context 예시](examples/05_active_context_example.md)
- [팀원용 1페이지 예시](examples/06_team_share_one_pager_example.md)
- [나쁜 예시와 좋은 예시](examples/07_bad_vs_good_manual_example.md)
- [Source Register 예시](examples/08_source_register_example.md)
- [품질 테스트 질문](tests/manual_quality_test.md)

---

## 8. 참고

- [참고 자료](references.md)
- [라이선스](LICENSE.md)

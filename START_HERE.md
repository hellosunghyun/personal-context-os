...
[목차](SUMMARY.md) | [배경 먼저 보기](docs/00_why_now.md) | [실행 흐름 →](workflow/00_linear_flow.md)
...
공개용 프로필, 업데이트 계획, GPTs/챗봇 확장, 월간 업데이트는 **완성 뒤 필요할 때** 봅니다.

이 가이드는 최신 기능을 다 챙겨 쓰는 법이 아닙니다. 기능이 바뀌어도 남는 원자료, 근거, 공개 범위, 저자성 검수 기준을 만드는 문서입니다.

그래서 이 가이드는 AI에게 더 많은 정보를 주자는 문서가 아니라, 어떤 정보를 줄지와 어떤 답변을 걸러낼지 정하는 문서입니다.
...
이 가이드는 대화가 필요한 단계마다 **텍스트 방식**과 **음성 방식**을 함께 안내합니다.
...
바로 시작하려면 [전체 실행 흐름](workflow/00_linear_flow.md)으로 갑니다. 왜 이 작업이 필요한지 먼저 보고 싶다면 [왜 지금 필요한가](docs/00_why_now.md)를 읽습니다.

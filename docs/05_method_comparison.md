...
|---|---|---|---|
| 개별 요청 다듬기 | 한 번의 요청을 잘 쓰기 | 사용자 맥락이 매번 빠질 수 있음 | 반복 맥락을 정리 |
| 맞춤 지시 | 짧은 선호 저장 | 깊은 사례와 검수 기록을 담기 어려움 | 원본 맥락을 압축해 제공 |
| 기억 | 과거 대화와 선호 반영 | 잘못되거나 오래된 기억이 남을 수 있음 | 기억할 것/지울 것 관리 |
| 프로젝트 | 파일과 대화 묶기 | 파일이 많아질수록 노이즈 증가 | Active Context로 핵심 압축 |
| 맞춤형 GPT | 반복 인터페이스 만들기 | 넣을 지식과 지시문 설계가 필요 | GPTs에 넣을 맥락 원본 제공 |
| 문서 검색 | 문서 검색 | 말투, 판단 기준, 저자성은 약함 | 사용자 맥락 정렬 |
| 에이전트 | 여러 단계 작업 자동화 | 잘못된 맥락으로 오래 실행될 수 있음 | 실행 전 기준과 경계 제공 |
